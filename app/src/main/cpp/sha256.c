...
/* -------------------------------------------------------------------------- */
static void _update(sha256_context *ctx)
{
    uint32_t b[8], *p, j;
...
/* -------------------------------------------------------------------------- */
void sha256_update(sha256_context *ctx, uint8_t *dat, uint32_t sz)
{
...
/* -------------------------------------------------------------------------- */
void sha256_final(sha256_context *ctx, uint8_t *buf)
{
    ...
    if (i < 56) i = (i >> 2) + 1;
    else ctx->buf[15] ^= (i < 60) ? ctx->buf[15] : 0, _update(ctx), i = 0;
...
int main(int argc, char *argv[])
{
    ...
    {
        sha256_init(&ctx);
        sha256_update(&ctx, (uint8_t *)buf[j], (uint32_t)strlen(buf[j]));
        sha256_final(&ctx, hv);
    ...
    for (j = 1; j < (uint32_t)argc; j++)
    {
        printf("argv[%d]: %s\nresult: ", (int)j, argv[j]);
        sha256_init(&ctx);
        sha256_update(&ctx, (uint8_t *)argv[j], (uint32_t)strlen(argv[j]));
        sha256_final(&ctx, hv);
        for (i = 0; i < 32; i++) printf("%02x%s", hv[i], ((i%4)==3)?" ":"");
        printf("\n\n");
    }
